...
module.exports = function (grunt) {
    ...
    ngconstant: {
      ...
      development: {
        ...
        constants: {
          ENV: {
            name: 'development',
            apiUrl: 'http://'+require('ip').address()+':9010/api',//数据在本地测试(mock)
            fake_token:"AXGNHBOXSFQBYNQH",
            isMock: true,
            isDebug: true
          }
        }
      ...
      online: {
        ...
        constants: {
          ENV: {
            name: 'online',
            apiUrl: 'http://tijian.techmars.cn/api',
            fake_token:"OJ07DZX4YGMKGWIT",
            isMock: false,
            isDebug: true
          }
        }
    ...
    da: {
      options: {
        ...
        uploaderOptions: {  //七牛相关的配置
          ...
          bucket:"tijian",
          domain: "7xnsso.com2.z0.glb.qiniucdn.com"
        },
        force: true,
  ...
  // Load the plugin that provides the "TiJian-angular" and "grunt-docular" tasks.
  // grunt.loadNpmTasks('grunt-TiJian-angular');
  ...
  grunt.registerTask('online', 'Compile then start online', function (target) {
    if (target === 'dist') {
      return grunt.task.run(['build', 'connect:dist:keepalive']);
    }
